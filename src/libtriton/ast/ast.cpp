...
namespace triton {
  namespace ast {
    ...
    FpsubNode::FpsubNode(const SharedAbstractNode& expr1, const SharedAbstractNode& expr2) : AbstractNode(FPSUB_NODE, expr1->getContext()) {
      this->addChild(expr1);
      this->addChild(expr2);
    }


    void FpsubNode::init(bool withParents) {
      if (this->children.size() < 2)
        throw triton::exceptions::Ast("FpsubNode::init(): Must take at least two children.");

      if (this->children[0]->getBitvectorSize() != this->children[1]->getBitvectorSize())
        throw triton::exceptions::Ast("FpsubNode::init(): Must take two nodes of same size.");

      if (this->children[0]->isArray() || this->children[1]->isArray())
        throw triton::exceptions::Ast("FpsubNode::init(): Cannot take an array as argument.");
    ...
    To_fp32Node::To_fp32Node(const SharedAbstractNode& expr) : AbstractNode(To_fp32_NODE, expr->getContext()) {
      this->addChild(expr);
    }


    void To_fp32Node::init(bool withParents) {
    ...
    void To_fp32Node::initHash(void) {
      triton::uint512 s = this->children.size();
    ...
    To_fp64Node::To_fp64Node(const SharedAbstractNode& expr) : AbstractNode(To_fp64_NODE, expr->getContext()) {
      this->addChild(expr);
    }


    void To_fp64Node::init(bool withParents) {
    ...
    void To_fp64Node::initHash(void) {
      triton::uint512 s = this->children.size();
    ...
    FpisZeroNode::FpisZeroNode(const SharedAbstractNode& expr) : AbstractNode(FpisZero_NODE, expr->getContext()) {
      this->addChild(expr);
    }


    void FpisZeroNode::init(bool withParents) {
    ...
    void FpisZeroNode::initHash(void) {
      triton::uint512 s = this->children.size();
    ...
    FpisPositiveNode::FpisPositiveNode(const SharedAbstractNode& expr) : AbstractNode(FpisPositive_NODE, expr->getContext()) {
      this->addChild(expr);
    }


    void FpisPositiveNode::init(bool withParents) {
    ...
    void FpisPositiveNode::initHash(void) {
      triton::uint512 s = this->children.size();
  ...
  namespace ast {
    ...
    static SharedAbstractNode shallowCopy(AbstractNode* node, bool unroll) {
      ...
      switch (node->getType()) {
        ...
        case FPSUB_NODE:                newNode = std::make_shared<FpsubNode>(*reinterpret_cast<FpsubNode*>(node));       break;
        case FpisPositive_NODE:         newNode = std::make_shared<FpisPositiveNode>(*reinterpret_cast<FpisPositiveNode*>(node)); break;
        case FpisZero_NODE:             newNode = std::make_shared<FpisZeroNode>(*reinterpret_cast<FpisZeroNode*>(node)); break;
        case To_fp32_NODE:              newNode = std::make_shared<To_fp32Node>(*reinterpret_cast<To_fp32Node*>(node));   break;
        case To_fp64_NODE:              newNode = std::make_shared<To_fp64Node>(*reinterpret_cast<To_fp64Node*>(node));   break;
